...
function mpause() {
    ...
    if [[ -z "$session_name" ]]; then
        if [[ -f "mutagen.yml" ]]; then
            # mutagen.yml에서 첫 번째 세션 이름 추출 (host-project 형식)
            session_name=$(grep -E "^  [^#[:space:]].*:" mutagen.yml | head -1 | sed 's/:$//' | sed 's/^  //')
            if [[ -z "$session_name" ]]; then
                # 기본 세션 이름도 시도
                session_name=$(grep -E "^  main:" mutagen.yml | head -1 | sed 's/:$//' | sed 's/^  //')
            fi
            if [[ -z "$session_name" ]]; then
                # 기본 세션 이름들 시도
